...
journey
    ...
    section 1. AI Discovery
      User snaps photo of old appliance: 5: User
      AI identifies model, age, efficiency: 5: System
      App reveals monthly ₹ wasted: 5: System
    section 2. Financial Catalyst
      Shows new Green Tech options: 4: System
      Auto-applies Govt/Partner Subsidies: 4: System
      User views "Payback Timer" (Net ROI): 5: User
    section 3. Zero-Risk Checkout
      App matches top-rated local installer: 4: System
      User selects 0% EMI financing: 5: User
      One-tap to book installation: 5: User
    section 4. The Last Mile (Partner Handled)
      E-Waste partner picks up old unit: 5: Partner
      Certified installer fits new unit: 5: Partner
      User earns "EcoCoins" & Social Badges: 5: System
```
...
graph TD;
    %% Frontend Clients
    Client_Mobile[Mobile App<br>Flutter: iOS & Android] --> API_Gateway
    
    %% API Gateway & Backend
    API_Gateway((FastAPI Gateway)) --> User_Service[User Auth & Profile]
    API_Gateway --> Vision_Engine[AI Analysis Engine<br>YOLOv8 + Gemini]
    API_Gateway --> Finance_Engine[Finance & Subsidy Engine]
    API_Gateway --> Lead_Router[Marketplace Lead Router]
    
    %% AI Models & Logic
    Vision_Engine --> Obj_Det[Obj Detection: YOLOv8]
    Vision_Engine --> LLM[Context Analysis: Gemini]
    Obj_Det --> Vision_Engine
    LLM --> Vision_Engine
    
    %% External Integrations
    Finance_Engine --> Ext_Banks((Banks & Gov DBs<br>Dummy/API))
    Lead_Router --> Ext_Installers((B2B Partners<br>BlueStar, Luminous))
    Lead_Router --> Ext_Ewaste((E-Waste Partners<br>Cashify))
    
    %% Database Layer
    User_Service --> DB[(Core Database<br>Supabase / PostgreSQL)]
    Finance_Engine --> DB
    Lead_Router --> DB
    
    %% Styling
    classDef highlight fill:#d4edda,stroke:#28a745,stroke-width:2px;
    class Vision_Engine,Finance_Engine highlight;
```
